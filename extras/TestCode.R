...
sql <- "
IF OBJECT_ID('@cohort_database_schema.@exposure_table', 'U') IS NOT NULL
	DROP TABLE @cohort_database_schema.@exposure_table;
...
sql <- SqlRender::renderSql(sql, 
                            ...
                            exposure_table = "mschuemi_test_exposures")$sql
sql <- SqlRender::translateSql(sql, targetDialect = connectionDetails$dbms, oracleTempSchema = oracleTempSchema)$sql
DatabaseConnector::executeSql(connection, sql)

x <- querySql(connection, "SELECT concept_id FROM cdm_truven_mdcd_v699.dbo.concept INNER JOIN cdm_truven_mdcd_v699.dbo.concept_ancestor ON concept_id = descendant_concept_id WHERE ancestor_concept_id = 21603933 AND concept_class_id = 'Ingredient'")
paste(x,collapse=", ")
